package com.alanogames.ane
{
	...
	public class Rating
	{
		 ...
		 * This method will immediately trigger the rating prompt without checking that the app store is available,
		 * and without calling the iRateShouldShouldPromptForRating delegate method. 
		 * Note that this method depends on the appStoreID and applicationGenre properties, 
		 * which are only retrieved after polling the iTunes server, so if you intend to call this method directly, 
		 * you will need to set these properties yourself beforehand, or use the promptIfNetworkAvailable method instead.
		 */		
		 ...
		 * This method will check if the app store is available, and if it is, it will display the rating prompt to the user. 
		 * The iRateShouldShouldPromptForRating delegate method will be called before the alert is shown, so you can intercept it. 
		 * Note that if your app is sandboxed and does not have the network access permission, this method will ignore the network availability status, 
		 * however in this case you will need to manually set the appStoreID or iRate cannot function.
		 */		
		 ...
		 * This method skips the user alert and opens the application ratings page in the Mac or iPhone app store, depending on which platform is running. 
		 * This method does not perform any checks to verify that the machine has network access or that the app store is available. 
		 * It also does not call any delegate methods. 
		 * You should use this method instead of the ratingsURL property if you are running on Mac OS as the process for launching the Mac app store is more complex than merely opening the URL. Note that this method depends on the appStoreID which is only retrieved after polling the iTunes server, 
		 * so if you intend to call this method directly, you will need to set the appStoreID property yourself beforehand.
		 */		
		 ...
		 * If set to YES, iRate will always display the rating prompt on launch, regardless of how long the app has been in use or whether it's the latest version.
		 *  Use this to proofread your message and check your configuration is correct during testing, but disable it for the final release (defaults to NO).
		 * @return 
		 * @default false
		 */
		 ...
		 * The time is measured from the first time the app is launched. 
		 * This is a floating point value, so it can be used to specify a fractional number of days (e.g. 0.5). The default value is 10 days. 
		 * @return 
		 * @default 10
		 */
		 ...
		 * This avoids the scenario where a user runs the app once, doesn't look at it for weeks and then launches it again, only to be immediately prompted to rate it. 
		 * The minimum use count ensures that only frequent users are prompted. 
		 * The prompt will appear only after the specified number of days AND uses has been reached. 
		 * This defaults to 10 events. This defaults to 10 uses.
		 * @return 
		 * @default 10
		 */		
		 ...
		 * If you are less concerned with the total number of times the app is used, but would prefer to use the frequency of times the app is used, 
		 * you can use the usesPerWeekForPrompt property to set a minimum threshold for the number of times the user must launch the app per week (on average) for the prompt to be shown. 
		 * Note that this is the average since the app was installed, so if the user goes for a long period without running the app, it may throw off the average. The default value is zero.
		 * @return 
		 * @default 0
		 */		
		 ...
		 * How long the app should wait before reminding a user to rate after they select the "remind me later" option (measured in days). 
		 * A value of zero means the app will remind the user next launch. 
		 * Note that this value supersedes the other criteria, so the app won't prompt for a rating during the reminder period, even if a new version is released in the meantime. 
		 ...
		 * This should match the iTunes app ID of your application, which you can get from iTunes connect after setting up your app. 
		 * This value is not normally necessary and is generally only required if you have the aforementioned conflict between bundle IDs for your Mac and iOS apps, 
		 * or in the case of Sandboxed Mac apps, if your app does not have network permission because it won't be able to fetch the appStoreID automatically using iTunes services.
		 * @return 
		 * 
		 */
		 ...
		 * This is the type of app, used to determine the default text for the rating dialog. 
		 * This is set automatically by calling an iTunes service, so you shouldn't need to set it manually for most purposes. 
		 * If you do wish to override this value, setting it to the iRateAppStoreGameGenreID constant will cause iRate to use the "game" version of the rating dialog, 
		 * and setting it to any other value will use the "app" version of the rating dialog.
		 * @return 
		 * 
		 */
		 ...
		 * This is the two-letter country code used to specify which iTunes store to check. 
		 * It is set automatically from the device locale preferences, so shouldn't need to be changed in most cases. 
		 * You can override this to point to the US store, or another specific store if you prefer, which may be a good idea if your app is only available in certain countries.
		 * @return 
		 * 
		 */
		 ...
		 * <br/>
		 * This is the name of the app displayed in the iRate alert. 
		 * It is set automatically from the application's info.plist, but you may wish to override it with a shorter or longer version. 
		 * @return 
		 * 
		 */
		 ...
		 * <br/> 
		 * This is the application bundle ID, used to retrieve the appStoreID and appStoreGenreID from iTunes. 
		 * This is set automatically from the app's info.plist, so you shouldn't need to change it except for testing purposes.
		 * @return 
		 * 
		 */
